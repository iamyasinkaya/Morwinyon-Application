...
using Morwiyon.Blazor.Iban.Exceptions;
using System.Text;
        ...
        private static string changeBbanEntry(string iban, string newValue, BBanEntryType entryType)
        {
            ...
            foreach (BBanEntry entry in structure.Entries)
            {
                if (entry.EntryType == entryType)
                {
                    ...
                    {
                        throw new IbanFormatException($"New value for {System.Enum.GetName(typeof(BBanEntryType), entry.EntryType)} is too long.", IbanFormatViolation.BBAN_ENTRY_TOO_LONG);
                    }
